...
export default class CookieConsent extends LightningElement {
  // State
  //@api displayType = "footer";modal
  @api displayType = "modal";
  ...
  connectedCallback() {
    ...
    // window.dispatchEvent(event);
     this.useRelaxedCSP =false;
    
    console.log('Cookie conset lwc connectedCallback');
    this.checkIfInPreview();
    if (this.useRelaxedCSP && !this.preview) {
      // console.log('Cookie conset lwc connectedCallback if');
      this.getBrowserIdCookie();
    } else if (!this.useRelaxedCSP && !this.preview) {
      // console.log('Cookie conset lwc connectedCallback else if');
      try {
        // console.log('Cookie conset lwc connectedCallback else if try');

        window.addEventListener("documentCookies", this.receiveCookiesFromHead.bind(this), false);
        //window.addEventListener("documentCookies", this.receiveCookiesFromHead(this), false);
      } catch (e) {
        console.log("error: " + e);
      }
      this.getCookiesFromHead();
    } else if (this.previewInBuilder) {
      // console.log('Cookie conset lwc connectedCallback else if 2');
  ...
  checkIfInPreview() {
    // console.log('Cookie conset lwc checkIfInPreview');
    ...
    // console.log('Cookie conset lwc checkIfInPreview urlToCheck : ' +urlToCheck);
    // console.log('Cookie conset lwc checkIfInPreview this.preview : ' +this.preview);
  ...
  receiveCookiesFromHead(e) {
    // console.log('cookieConsent receiveCookiesFromHead');
    this.uniqueId = e.detail;
    // console.log('cookieConsent receiveCookiesFromHead e : ', e);
    // console.log('cookieConsent receiveCookiesFromHead this.uniqueId : ', this.uniqueId);
    if (this.uniqueId) {
      this.verifyBrowserIdWithUniqueId();
  ...
  verifyBrowserIdWithUniqueId() {
    // console.log('cookieConsent verifyBrowserIdWithUniqueId this.uniqueId : ', this.uniqueId);
    console.log('cookieConsent verifyBrowserIdWithUniqueId this.uniqueId : ', this.uniqueId);

    verifyBrowserId({ browserId: this.uniqueId })
      .then(data => {
        console.log('cookieConsent verifyBrowserIdWithUniqueId data : ', data);
        if (data === false) {
        ...
        // console.log('cookieConsent verifyBrowserIdWithUniqueId this.showCookieDialog : ', this.showCookieDialog);
      })
      .catch(error => {
        this.error = error.message;
        console.log('cookieConsent verifyBrowserIdWithUniqueId this.error : ', this.error);
      });
  }
